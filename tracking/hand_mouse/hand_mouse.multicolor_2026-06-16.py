...
mouse_state = {
    ...
    "needs_save": False,
    "corner_capture": None,  # als een lijst: we verzamelen 4 handmatige schermhoeken (cam-px)
}

def on_mouse(event, x, y, flags, param):
    state = param
    state["pos"] = (x, y)
    # Handmatige hoek-selectie heeft voorrang: elke klik = één schermhoek (max 4).
    cc = state.get("corner_capture")
    if cc is not None:
        if event == cv2.EVENT_LBUTTONDOWN and len(cc) < 4:
            cc.append((x, y))
        return
...
# Kleuren (BGR) die per stip achtereenvolgens geprobeerd worden. Een witte
# stip wast soms weg op lichte/heldere projectie-ondergrond; felle kleuren
# geven dan in hun eigen kanaal alsnog een sterk verschil. Volgorde = van
# meest- naar minst-betrouwbaar in de praktijk.
AUTO_CAL_DOT_COLORS = [
    (255, 255, 255),  # wit
    (0, 255, 255),    # geel
    (255, 255, 0),    # cyaan
...
def _render_dot(display, screen_xy_norm, idx, total, color=(255, 255, 255)):
    img = _render_black(display)
    W, H = display["w"], display["h"]
    cx = int(screen_xy_norm[0] * W)
    cy = int(screen_xy_norm[1] * H)
    cv2.circle(img, (cx, cy), AUTO_CAL_DOT_RADIUS, color, -1, cv2.LINE_AA)
...
def _find_dot_blob(frame_on, baseline, expected_px=None, search_radius=None):
    """Vind de stip-blob via per-kanaal diff. Returns (cx, cy) of None.

    Per-kanaal max-diff i.p.v. grayscale: een felle gekleurde stip geeft in
    zijn eigen kanaal een sterk verschil, ook als de grijswaarde nauwelijks
    verandert (bv. gele stip op lichte ondergrond)."""
    diff = cv2.absdiff(frame_on, baseline)
    # Max over de 3 kanalen → vangt elke kleur-stip.
    ...
    if expected_px is not None and search_radius is not None:
        ...
        return (near[0][0][0], near[0][0][1])
    candidates.sort(key=lambda c: -c[2])
    return (candidates[0][0], candidates[0][1])


def _detect_dot(cap, display, mirror_x, baseline, screen_xy, idx, total, expected_px=None, search_radius=None):
    """Project een stip en detecteer hem. Probeert MEERDERE kleuren per stip
    zodat stippen die in wit wegvallen alsnog in een andere kleur worden
    gevonden. Returns ((cx, cy), None) of (None, reden)."""
    last_reason = "no bright region"
    for color in AUTO_CAL_DOT_COLORS:
        cv2.imshow(CALIB_WIN, _render_dot(display, screen_xy, idx, total, color))
        cv2.waitKey(int(AUTO_CAL_FRAME_GRACE_S * 1000))
        for _ in range(AUTO_CAL_SETTLE_FRAMES):
            cap.grab()
        frame_on = _capture(cap, mirror_x)
        cv2.waitKey(int(AUTO_CAL_POST_HOLD_S * 1000))
        if frame_on is None:
            last_reason = "camera dropped frame"
            continue
        hit = _find_dot_blob(frame_on, baseline, expected_px, search_radius)
        if hit is not None:
            return hit, None
        last_reason = "geen blob in geen enkele kleur"
    return None, last_reason

def capture_corners_interactive(cap, mirror_x):
    """Laat de gebruiker de 4 schermhoeken in de PREVIEW aanklikken (TL, TR, BR, BL).

    Returns 4 hoeken in genormaliseerde cam-coords (0..1), of None bij annuleren.
    Gebruikt de bestaande on_mouse-callback via mouse_state['corner_capture'].
    """
    labels = ["1/4 LINKSBOVEN (TL)", "2/4 RECHTSBOVEN (TR)",
              "3/4 RECHTSONDER (BR)", "4/4 LINKSONDER (BL)"]
    mouse_state["corner_capture"] = []
    cc = mouse_state["corner_capture"]
    print("[calib] Klik de 4 schermhoeken in de preview: TL, TR, BR, BL. "
          "(u=undo, ENTER/SPATIE=bevestig, ESC=annuleer)")
    try:
        while True:
            ok, frame = cap.read()
            if not ok:
            ...
            if mirror_x:
                frame = cv2.flip(frame, 1)
            h, w = frame.shape[:2]
            # Reeds geklikte hoeken + verbindingslijnen tekenen.
            for j, (px, py) in enumerate(cc):
                cv2.circle(frame, (int(px), int(py)), 11, (0, 255, 0), 2, cv2.LINE_AA)
                cv2.circle(frame, (int(px), int(py)), 2, (0, 255, 0), -1, cv2.LINE_AA)
                cv2.putText(frame, str(j + 1), (int(px) + 13, int(py) + 5),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2, cv2.LINE_AA)
            if len(cc) >= 2:
                cv2.polylines(frame, [np.array(cc, np.int32).reshape(-1, 1, 2)],
                              len(cc) == 4, (0, 200, 0), 2)
            n = len(cc)
            if n < 4:
                cv2.putText(frame, f"Klik schermhoek {labels[n]}", (20, 45),
                            cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2, cv2.LINE_AA)
                cv2.putText(frame, "klik op de HOEK van het geprojecteerde scherm   "
                            "u=undo  ESC=annuleer", (20, 82),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 1, cv2.LINE_AA)
            else:
                cv2.putText(frame, "4 hoeken gezet. ENTER/SPATIE = kalibreren", (20, 45),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2, cv2.LINE_AA)
                cv2.putText(frame, "u=undo  ESC=annuleer", (20, 82),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 1, cv2.LINE_AA)
            cv2.imshow(PREVIEW_WIN, frame)
            k = cv2.waitKey(20) & 0xFF
            if k == 27:                     # ESC
                return None
            elif k == ord('u') and cc:      # undo laatste hoek
                cc.pop()
            elif len(cc) == 4 and k in (13, 32):  # ENTER / SPATIE
                return [(px / max(1, w), py / max(1, h)) for (px, py) in cc]
    finally:
        mouse_state["corner_capture"] = None


def auto_calibrate(cap, display, mirror_x, cols, rows, corner_cam_norm=None):
    """Structured-light kalibratie. Returns (samples, cols, rows) of (None, cols, rows).

    corner_cam_norm: optioneel, 4 schermhoeken in genormaliseerde cam-coords
        (0..1), volgorde TL, TR, BR, BL, handmatig aangeklikt in de preview.
        Indien gegeven slaan we de fragiele auto-detectie van Phase 1 over en
        bouwen we de homografie direct uit deze hoeken.

    Phase 1 (alleen bij corner_cam_norm=None): 4 buitenhoeken auto-detecteren.
    Phase 2: per grid-punt de verwachte cam-positie via de homografie, dan de
             stip detecteren in een ruime zone rond dat punt (alle kleuren).
    """
    ...
    # Schermhoeken in projector-coords (0..1): TL, TR, BR, BL.
    corner_screen = [(0.0, 0.0), (1.0, 0.0), (1.0, 1.0), (0.0, 1.0)]

    if corner_cam_norm is not None:
        # --- Handmatige hoeken (aangeklikt in de preview) ---
        corner_cam = [(nx * w, ny * h) for (nx, ny) in corner_cam_norm]
        print("[calib] MANUELE hoeken gebruikt (TL,TR,BR,BL):",
              [(round(nx, 3), round(ny, 3)) for nx, ny in corner_cam_norm])
    else:
        # --- Phase 1: detecteer de 4 buitenhoeken automatisch ---
        ...
        for i, sxy in enumerate(corner_screen):
            ...
            if pos is None:
                ...
                cv2.imshow(CALIB_WIN, _render_message(display,
                    ...
                    sub="Gebruik handmatige hoeken (klik ze in de preview met 'a')."))
    ...
    # Ruwe screen->camera homografie uit de 4 hoeken (handmatig of gedetecteerd).
    dst_cam = [(p[0]/w, p[1]/h) for p in corner_cam]
    H_screen_to_cam = compute_homography_4pt(corner_screen, dst_cam)

    # Zoekradius. Er wordt per frame maar ÉÉN stip geprojecteerd, dus een ruime
    # radius is veilig (geen buur-stip om te verwarren). big_radius = "beduidend
    # groter" als fallback, zoals gevraagd.
    proj_diag = ((corner_cam[2][0] - corner_cam[0][0]) ** 2 +
                 (corner_cam[2][1] - corner_cam[0][1]) ** 2) ** 0.5
    cell = proj_diag / max(cols, rows)
    search_radius = max(45.0, 0.9 * cell)
    big_radius = max(search_radius * 4.0, 0.40 * proj_diag)
    ...
    for i, sxy in enumerate(targets):
        ...
        # Poging 1: normale (ruime) radius.
        pos, err = _detect_dot(cap, display, mirror_x, baseline, sxy, i, len(targets),
                                expected_px=(ex_px, ey_px), search_radius=search_radius)
        # Poging 2: 2x ruimer.
        if pos is None:
            print(f"[calib] dot {i+1} mis ({err}) — ruimere radius")
            time.sleep(0.2)
            pos, err = _detect_dot(cap, display, mirror_x, baseline, sxy, i, len(targets),
                                    expected_px=(ex_px, ey_px),
                                    search_radius=search_radius * 2.0)
        # Poging 3: BEDUIDEND grotere radius (er staat toch maar 1 stip aan).
        if pos is None:
            print(f"[calib] dot {i+1} nog steeds mis — grote radius ({int(big_radius)}px)")
            time.sleep(0.2)
            pos, err = _detect_dot(cap, display, mirror_x, baseline, sxy, i, len(targets),
                                    expected_px=(ex_px, ey_px),
                                    search_radius=big_radius)
        # Laatste redmiddel: schat de positie via de homografie zodat de
        # kalibratie afmaakt (deze cel wordt iets minder nauwkeurig).
        if pos is None:
            print(f"[auto-calib] dot {i+1} undetectable — using estimated position")
            pos = (ex_px, ey_px)
...
def draw_preview(frame, landmarks, gesture_name, ctx):
    ...
    hud_lines = [
        ...
        "a = KALIBREER: klik 4 schermhoeken (TL,TR,BR,BL), dan auto (alle kleuren)",
        "of DRAG points met de muis om het mesh handmatig bij te stellen",
            ...
            if not args.no_preview:
                ...
                if key == ord('q'):
                    ...
                    if grid_cols > AUTO_CAL_MAX_GRID or grid_rows > AUTO_CAL_MAX_GRID:
                        print(f"[calib] grid was {grid_cols}×{grid_rows} — cap op {AUTO_CAL_MAX_GRID}×{AUTO_CAL_MAX_GRID}")
                    # Stap 1: klik de 4 schermhoeken in de preview.
                    corners = capture_corners_interactive(cap, mirror_x)
                    if corners is None:
                        print("[calib] hoek-selectie geannuleerd — kalibratie niet uitgevoerd")
                    else:
                        # Stap 2: Phase 2 met die hoeken (alle kleuren + grote fallback-radius).
                        print("[calib] starten — verlaat zo het beeld voor de stippen-meting!")
                        new_samples, new_cols, new_rows = auto_calibrate(
                            cap, display, mirror_x, grid_cols, grid_rows,
                            corner_cam_norm=corners)
                        if new_samples is not None and len(new_samples) == new_cols * new_rows:
                            ...
                            save_calibration(calib_samples, grid_cols, grid_rows, mirror_x)
                            calib_present = True
                            print(f"[calib] KLAAR — {grid_cols}×{grid_rows} grid gekalibreerd via handmatige hoeken")
                        else:
                            print("[calib] mislukt — vorige kalibratie behouden. Tip: donkerder, "
                                  "of klik de hoeken nauwkeuriger.")
                elif key == ord('r'):
